...
export type CartItem = {
  ...
  lineId: string | null;
  /** Local route slug, used for links inside the Fun Taleem UI */
  slug: string;
  handle: string;
  productId: string;
  variantId: string;
  ...
  persist(
    (set, get) => ({
      ...
      addBySlug: async (slug, quantity = 1) => {
        const { PRODUCT_HANDLES } = await import("@/lib/shopify");
        const handle = PRODUCT_HANDLES[slug];
        if (!handle) return;
        set({ isLoading: true });
        let item: AddInput | null = null;
        try {
          const product = await fetchProductByHandle(handle);
          if (product) item = cartItemFromProduct(slug, product, quantity);
        } catch (error) {
          console.error("Failed to load product from Shopify:", error);
        } finally {
          set({ isLoading: false });
        }
        if (item) await get().addItem(item);
      },
...
export function cartTotals(items: CartItem[]) {
  ...
  for (const item of items) {
    qty += item.quantity;
    const unit = toAmount(item.price);
    const compare = item.compareAtPrice ? toAmount(item.compareAtPrice) : unit;
    subtotal += unit * item.quantity;
    compareTotal += compare * item.quantity;
  }
  return { qty, subtotal, compareTotal };
}
