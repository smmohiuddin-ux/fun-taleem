...
const serverPublicDirectory = new URL("../server/public/", import.meta.url);
...
await cp(new URL("site-assets/", output), new URL("site-assets/", serverPublicDirectory), {
  recursive: true,
});
await cp(new URL("lov-assets/", output), new URL("lov-assets/", serverPublicDirectory), {
  recursive: true,
});
...
await writeFile(new URL("index.html", serverPublicDirectory), html);
...
console.log("Hostinger deployment files published to the repository root and server/public.");